...
def resolve_candidate_directory(
    ...
    """
    Resolve a candidate path string to a directory path.
    ...
    # Strip leading slashes - treat /path/to/dir as root-relative, not absolute
    normalized = normalized.lstrip("/")
    ...
    try:
        resolved = (source_dir / candidate_path).resolve()
        if resolved.is_dir() and _is_within_repo(resolved, root):
            return resolved
    except (OSError, ValueError):
        pass
